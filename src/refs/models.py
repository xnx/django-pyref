...
class Ref(models.Model):
    ...
    def __str__(self):
        ...
        return "{}: {}, {}".format(
            self.id, self._get_or_missing("authors"), self._get_or_missing("title")
        )
    ...
    def _make_url_html(self, s_url=None):
        ...
        if s_url:
            ...
            return '<span class="noprint"> [<a href="{}">{}' "</a>]</span>".format(
                s_url, url_or_doi
            )
        return ""
    ...
    def html_article(self, pk=True, authors_nmax=None):
        ...
        s = (
            "{s_pk}{authors}{title}, {journal}{volume}{pages}"
            "{article_number} {year}. {url}".format(
                s_pk=s_pk,
                authors=s_authors,
                title=s_title,
                journal=s_journal,
                volume=s_volume,
                pages=s_pages,
                article_number=s_article_number,
                year=s_year,
                url=s_url,
            )
        )
...
def get_bibcode(doi):
    url = requests.get(
        ...
        params={"q": 'doi:"{}"'.format(doi), "fl": "*", "rows": 2000},
...
def get_title_from_bibcode(bibcode, fmt="html"):
    ...
    if "error" in response_json:
        raise ADSError(
            "ADS service returned an error: {}".format(response_json["error"])
        )
    try:
